# `/migration-ui` Framework Realignment Plan
...
`/migration-ui` ya no debe tratarse como una superficie de `App/Surface` ni como un runtime aislado de prueba.
...
- `Repository/App/Surface/MigrationUi`

Wiring actual:

- namespace PHP: `App\Surface\MigrationUi\...`
- rutas: `Repository/App/Surface/MigrationUi/routes.php`
- contrato adicional: `Repository/App/Surface/MigrationUi/module.php`
- vistas: `Repository/App/Surface/MigrationUi/Views`
- assets fuente: `Repository/App/Surface/MigrationUi/front/*`
- previews estaticos: `Repository/App/Surface/MigrationUi/generated/theme-previews/*`
- asset runtime publicado: `public/assets/js/work/migrationui/script.js`

Runtime actual:

- `boot-core/template/components/_catalyst-init.phtml` inicializa `public/assets/js/catalyst/catalyst.js`
- `boot-core/template/layouts/migration-ui-shell.phtml` carga ademas `public/assets/js/work/migrationui/script.js`
...
- `/migration-ui` sigue siendo un runtime concentrado en un script de surface
...
`/migration-ui` debe pasar a `Repository/Framework`.
...
- romper carga de vistas si `View::addPath(...)` sigue apuntando a `Repository/App/Surface/MigrationUi/Views`
...
- `migration-ui-shell` ya vive en `boot-core/template/layouts/`, fuera del modulo
- las rutas publicas pueden mantenerse:
  - `/migration-ui`
  - `/migration-ui/basic-elements`
...
- cualquier referencia literal a `Repository/App/Surface/MigrationUi`
...
- `Repository/Framework/MigrationUi`

Cambios obligatorios:

- `namespace App\Surface\MigrationUi\...` -> `namespace Catalyst\Repository\MigrationUi\...`
- `use App\Surface\MigrationUi\Controllers\MigrationUiController` -> `use Catalyst\Repository\MigrationUi\Controllers\MigrationUiController`
- `View::addPath(...)` apuntando a `Repository/Framework/MigrationUi/Views`
- `loadThemePreviewHtml()` apuntando a `Repository/Framework/MigrationUi/generated/theme-previews`
...
- el modulo ya vive en `Repository/Framework/MigrationUi`
- el controlador ya usa `namespace Catalyst\Repository\MigrationUi\Controllers`
- `routes.php` ya referencia el namespace nuevo
- `View::addPath(...)` ya apunta a `Repository/Framework/MigrationUi/Views`
- la carga de previews ya apunta a `Repository/Framework/MigrationUi/generated/theme-previews`

### Fase 2 — desacoplar runtime del surface

Estado: `ejecucion inicial completada`

Extraer desde `public/assets/js/work/migrationui/script.js` hacia modulos nativos bajo `public/assets/js/catalyst/modules/`.
...
- `Repository/Framework/MigrationUi/front/script.js` ya no contiene la logica pesada; ahora solo hace bootstrap opt-in del shell
...
- las sobreescrituras del tema quedan en `Repository/Framework/MigrationUi/front/style.css` y deben cargar despues del CSS base del plugin

Pendiente dentro de la misma fase:

- revisar si alguna parte de `card-actions` y `code-preview` debe renombrar clases residuales `migration-ui-*`
...
Dejar `public/assets/js/work/migrationui/script.js` solo como bootstrap opt-in.
...
- mantener temporalmente `work/migrationui`
- o migrar luego a un slug mas canonico cuando el shell quede estabilizado

Recomendacion:

- conservar temporalmente `work/migrationui`
...
4. convertir `work/migrationui/script.js` en bootstrap minimo
...
- `/migration-ui` y `/migration-ui/basic-elements` siguen resolviendo
- no queda namespace `App\\Surface\\MigrationUi`
- no queda path runtime apuntando a `Repository/App/Surface/MigrationUi`
...
- abrir `https://catalyst.dock/migration-ui`
- abrir `https://catalyst.dock/migration-ui/basic-elements`
