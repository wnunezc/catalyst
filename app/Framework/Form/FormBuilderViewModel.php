...
namespace Catalyst\Framework\Form;
...
final class FormBuilderViewModel
{
    ...
    $stringifyAttributes = static function (array $attributes): string {
        ...
        foreach ($attributes as $key => $value) {
            $key = trim((string) $key);
            if (
                preg_match('/^[A-Za-z_:][A-Za-z0-9:._-]*$/', $key) !== 1
                || str_starts_with(strtolower($key), 'on')
            ) {
                continue;
            }
    ...
    if ($hasDependencies) {
        $formAttrs .= ' data-form-dependencies="1"';
    }
    if ($hasRepeaters) {
        $formAttrs .= ' data-form-repeaters="1"';
    }
    if (!empty($autosave['enabled'])) {
        $formAttrs .= ' data-form-autosave="1"';
    }
    if (!empty($autosave['key'])) {
        $formAttrs .= ' data-form-autosave-key="' . e((string) $autosave['key']) . '"';
    }
