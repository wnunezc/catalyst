...
class Translator
{
     ...
     *
     * Must be called once in Kernel::bootstrap(), after SessionManager::init()
     * so that getLocale() can read the session.
     *
    ...
    /**
     * Register an additional lang/ directory (called from module routes.php).
     *
     * Paths registered later override earlier ones on key collision,
     * so modules can override global translations for their own group files.
     *
    ...
    /**
     * Set the user's locale preference (stored in session).
     *
     * Does not require the user to be logged in. The locale persists
     * for the session duration regardless of authentication state.
     *
     ...
     *
     * Resolution order: session → default locale from .env.
     *
     ...
     *
     * Key format: '{group}.{subkey}' where group = JSON file name.
     *   'validation.required'      → loads validation.json → key 'required'
     *   'messages.save_success'    → loads messages.json   → key 'save_success'
     *   'dates.formats.default'    → loads dates.json      → nested 'formats.default'
     *
     * Fallback chain:
     *   1. Requested locale
     *   2. Default locale (if different from requested)
     *   3. The key itself (no exception thrown)
     *
    ...
    /**
     * Get an array value for use as select options or enumeration lists.
     *
     * Only returns entries where the value is a string (skips nested arrays),
     * making it safe to pass directly to a <select> builder.
     *
     * JSON example (form.json):
     *   "gender_options": { "male": "Male", "female": "Female", "other": "Other" }
     *   "gender_default": "male"
     *
     * Usage:
     *   $options = Translator::getInstance()->getList('form.gender_options');
     *   // → ['male' => 'Male', 'female' => 'Female', 'other' => 'Other']
     *
     *   $default = __('form.gender_default');
     *   // → 'male'
     *
    ...
    /**
     * Format a date using a locale-aware format pattern.
     *
     * Format patterns are defined in dates.json under 'formats.*'.
     * Month and day names are translated using the dates.json name tables.
     *
     * Built-in format keys (defined in boot-core/lang/{locale}/dates.json):
     *   'default'  → locale default (e.g. 'm/d/Y' for en, 'd/m/Y' for es)
     *   'long'     → full month name (e.g. 'January 15, 2026')
     *   'short'    → abbreviated (e.g. '01/15/26')
     *   'full'     → day name + full date
     *   'time'     → time only
     *   'datetime' → date + time
     *   'iso'      → Y-m-d (locale-independent)
     *
    ...
    /**
     * Parse a dot-notation key into [group, subkey].
     *
     *   'validation.required'      → ['validation', 'required']
     *   'dates.formats.default'    → ['dates', 'formats.default']
