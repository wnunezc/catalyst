...
/**
 * Migrates persisted Media module overrides to the canonical Workspaces owner.
 *
...
return new class extends Migration {
    private const string SOURCE = 'module.framework.media';
    private const string TARGET = 'module.framework.workspaces';

    public function getVersion(): string
    {
        return '20260612020000';
    }

    public function up(): void
    {
        $this->statement(
            'CREATE TABLE IF NOT EXISTS `workspaces_feature_flag_migration` (
        ...
        $this->connection()->transaction(function (Connection $connection): void {
            ...
            foreach ($rows as $row) {
                ...
                $journal = $connection->selectOne(
                    'SELECT source_id FROM workspaces_feature_flag_migration WHERE source_id = ?',
                    [(int) $row['id']]
                );
                if ($journal === null) {
                    $connection->insert('workspaces_feature_flag_migration', [
    ...
    public function down(): void
    {
        $this->connection()->transaction(function (Connection $connection): void {
            $rows = $connection->select(
                'SELECT * FROM workspaces_feature_flag_migration ORDER BY source_id'
            );
        ...
        $this->statement('DROP TABLE IF EXISTS `workspaces_feature_flag_migration`');
    }
};
