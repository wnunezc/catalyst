...
return static function (array $scope): array {
    ...
    $migrationCssPath = implode(DS, [PD, 'public', 'assets', 'css', 'work', 'migrationui', 'style.css']);
    $migrationJsPath = implode(DS, [PD, 'public', 'assets', 'js', 'work', 'migrationui', 'script.js']);
    $migrationCssVersion = (string) (@filemtime($migrationCssPath) ?: time());
    $migrationJsVersion = (string) (@filemtime($migrationJsPath) ?: time());
    ...
    $appendStyle('/assets/css/work/migrationui/style.css?v=' . rawurlencode($migrationCssVersion));
...
$appendStyle('/assets/css/catalyst/inspinia-runtime-compat.css?v=' . rawurlencode((string) (@filemtime(implode(DS, [PD, 'public', 'assets', 'css', 'catalyst', 'inspinia-runtime-compat.css'])) ?: time())));
    ...
    $appendScript('/assets/js/work/migrationui/script.js?v=' . rawurlencode($migrationJsVersion), true);

    if ($moduleSlug !== '' && $moduleSlug !== 'migrationui') {
    ...
    return [
        ...
        'migration_nav_groups' => is_array($scope['migration_nav_groups'] ?? null) && $scope['migration_nav_groups'] !== [] ? $scope['migration_nav_groups'] : $navGroups,
        ...
        'migration_ui_page_slug' => (string) ($scope['migration_ui_page_slug'] ?? trim($currentPath, '/')),
