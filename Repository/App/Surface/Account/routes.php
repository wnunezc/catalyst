...
use App\Surface\Account\Controllers\AccountCenterController;
use App\Surface\Account\Controllers\AccountRecoveryAdminController;
use App\Surface\Account\Controllers\AccountRecoveryController;
...
View::getInstance()->addPath(
    'account',
    implode(DS, [PD, 'Repository', 'App', 'Surface', 'Account', 'Views'])
);

Translator::getInstance()->addPath(
    implode(DS, [PD, 'Repository', 'App', 'Surface', 'Account', 'lang'])
);
...
$accountRecoveryAdminMiddleware = [
    AuthMiddleware::class,
    new RoleMiddleware(permissions: 'manage-account-recovery'),
];

$router->get('/admin/account-recovery', [AccountRecoveryAdminController::class, 'index'])
       ->middleware($accountRecoveryAdminMiddleware);
$router->get('/admin/account-recovery/{id}', [AccountRecoveryAdminController::class, 'show'])
       ->middleware($accountRecoveryAdminMiddleware);
$router->post('/admin/account-recovery/{id}/approve', [AccountRecoveryAdminController::class, 'approve'])
       ->middleware($accountRecoveryAdminMiddleware)
       ->throttle('admin_mutation');
$router->post('/admin/account-recovery/{id}/reject', [AccountRecoveryAdminController::class, 'reject'])
       ->middleware($accountRecoveryAdminMiddleware)
       ->throttle('admin_mutation');
