...
/**
 * Provisions the initial privileged account required by environment setup.
 *
 * @package Catalyst\Repository\Configuration\Services
 * Responsibility: Detects active privileged accounts, ensures the required role and creates the first privileged account.
 */
final class SetupPrivilegedAccountProvisioner
{
    /**
     * Initializes the setup privileged account provisioner instance.
     *
     * Responsibility: Initializes the setup privileged account provisioner instance.
     */
...
public static function make(): self
    ...
    /**
     * Determines whether an active privileged account exists.
     *
     * Responsibility: Determines whether an active privileged account exists.
 */
public function privilegedAccountExists(PDO $pdo): bool
    {
        try {
            $stmt = $pdo->query(
        ...
        try {
            ...
            $pdo->prepare(
                "INSERT INTO roles (tenant_id, name, slug, description)
                 VALUES (1, 'Administrator', 'admin', 'System privileged role')"
            )->execute();

            return (int) $pdo->lastInsertId();
        } catch (Throwable $e) {
            $this->logger->error('SetupPrivilegedAccountProvisioner: required role lookup failed', [
                'error' => $e->getMessage(),
            ]);

            throw new RuntimeException('Unable to ensure required role: ' . $e->getMessage(), 0, $e);
        }
    }

    /**
     * Creates the privileged account and role assignment atomically.
     *
     * Responsibility: Creates the privileged account.
 */
public function createPrivilegedAccount(PDO $pdo, string $name, string $email, string $password): void
    {
        ...
        try {
            ...
            $this->logger->error('SetupPrivilegedAccountProvisioner: account creation failed', [
                'error' => $e->getMessage(),
            ]);
