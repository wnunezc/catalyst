...
namespace Catalyst\Repository\MigrationUi\Controllers;
...
final class MigrationUiController extends Controller
{
     ...
     */
    private function migrationUiSections(): array
    {
        return [
    ...
    {
        $sections = $this->migrationUiSections();
    ...
    {
        $frontDir = PD . DS . 'Repository' . DS . 'Framework' . DS . 'MigrationUi' . DS . 'front';
        ...
        return $this->view('migrationui.migration-ui', [
            'title' => 'Demo UI',
            'pageTitle' => 'Demo UI',
            'csp_nonce' => CspNonce::get(),
            'auth_name' => trim((string) ($authUser['name'] ?? '')),
            'status_bar_show_theme_toggle' => true,
            'status_bar_theme_toggle_attribute' => 'data-migrationui-theme-toggle',
            ...
            'styles' => [
                ...
                ...$pageStyles,
                '/assets/css/work/migrationui/style.css?v=' . rawurlencode($styleVersion),
            ],
            'scripts' => [
                [
                    'src' => '/assets/js/work/migrationui/script.js?v=' . rawurlencode($scriptVersion),
                    'defer' => true,
                ],
            ],
            'migration_nav_groups' => $navGroups,
            ...
            'migration_ui_page_slug' => $pageSlug ?? '',
            'preview_html' => TrustedHtml::fromString($this->loadThemePreviewHtml($selectedFile)),
        ], 200, 'migration-ui-shell')->setAttribute('csp_profile', 'trusted-renderer');
    }
    ...
    private function buildNavItems(array $items, string $selectedFile): array
    {
        ...
        foreach ($items as $item) {
            ...
            $navItems[] = [
                ...
                'is_active' => $isActive,
                'link_class' => $isActive ? 'migration-ui-nav__link active' : 'migration-ui-nav__link',
            ];
        }
    ...
    private function buildChartNavItems(string $selectedFile): array
    {
        ...
        foreach (self::CHART_FAMILIES as $family => $definition) {
            ...
            $items[] = [
                ...
                'collapse_id' => 'migration-charts-' . $family,
    ...
    private function buildTableNavItems(string $selectedFile): array
    {
        ...
        foreach (self::TABLE_FAMILIES as $family => $definition) {
            ...
            $items[] = [
                ...
                'is_active' => $isFamilyActive,
                'link_class' => $isFamilyActive ? 'side-nav-link active' : 'side-nav-link',
                'collapse_id' => 'migration-tables-' . $family,
                'expanded' => $isFamilyActive ? 'true' : 'false',
                'show' => $isFamilyActive,
                'children' => $childItems,
            ];
    ...
    {
        $previewPath = PD . DS . 'Repository' . DS . 'Framework' . DS . 'MigrationUi' . DS . 'generated' . DS . 'theme-previews' . DS . basename($file);
