...
final class SetupAdminProvisionerTest extends TestCase
{
    ...
    public function testCreatesActiveVerifiedAdminAndRoleAssignmentAtomically(): void
    {
        $pdo = $this->database(true);
        $service = new SetupAdminProvisioner(Logger::getInstance(), ConfigManager::getInstance());

        $service->createAdmin($pdo, 'Initial Admin', 'admin@example.com', 'correct-password');

        $user = $pdo->query('SELECT * FROM users')->fetch(PDO::FETCH_ASSOC);
        $assignment = $pdo->query('SELECT * FROM user_roles')->fetch(PDO::FETCH_ASSOC);
        Assert::same('admin@example.com', $user['email'] ?? null);
    ...
    public function testRollsBackUserAndRoleWhenAssignmentFails(): void
    {
        ...
        Assert::true(false, 'Expected role assignment failure to roll back setup admin creation.');
    }
