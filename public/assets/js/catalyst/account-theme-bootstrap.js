(function () {
    ...
    var appearanceNode = document.getElementById('account-appearance-json');
    var appearance = {};

    try {
        appearance = appearanceNode ? JSON.parse(appearanceNode.textContent || '{}') : {};
    } catch (_) {
        appearance = {};
    }
    ...
    function getClosedSkinConfig(skin) {
        return Object.prototype.hasOwnProperty.call(closedSkins, skin) ? closedSkins[skin] : null;
    }
    ...
    function sanitize(raw) {
        ...
        return enforceClosedSkinConfig({
            skin: pick(source.skin, defaults.skin, allowed.skin),
            theme: pick(source.theme, defaults.theme, allowed.theme),
            'topbar-color': pick(source['topbar-color'], defaults['topbar-color'], allowed['topbar-color']),
            'sidenav-color': pick(source['sidenav-color'], defaults['sidenav-color'], allowed['sidenav-color']),
    ...
    function lockedConfig() {
        ...
        return sanitize(Object.assign({}, defaults, candidate && typeof candidate === 'object' ? candidate : {}));
    }

    var config = isLocked() ? lockedConfig() : sanitize(Object.assign({}, currentMarkupConfig(), readStoredConfig() || {}));
