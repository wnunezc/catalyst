...
function bindChartGlobals() {
    ...
    window.__migrationUiChartsVendorBase = '/assets/vendor/inspinia';
...
function resolveChartDocument(root) {
    const shell = root instanceof HTMLElement
        ? root.closest('.migration-ui-shell-body')
        : null;
    ...
    return source.dataset.migrationuiDoc ?? '';
}
...
async function loadChartDependencies(docFile) {
    if (docFile.startsWith('charts-apex-')) {
        await ensureScript('/assets/vendor/inspinia/plugins/apexcharts/apexcharts.min.js');
        return;
    }

    if (!docFile.startsWith('charts-echart-')) {
        return;
    }
...
export async function initMigrationUiCharts(options = {}) {
    ...
    bindChartGlobals();
    await loadChartDependencies(docFile);
