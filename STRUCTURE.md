...
- **Settings module** (`Repository/Framework/Settings/`) — Config Panel (Etapa 7): ConfigController (GET + effective setup/runtime state), HealthController (`/health`, `/health/live`, `/health/ready`), dedicated setup save controllers for `/setup/app|db|mail|session|cache|logging|security|websocket|devtools` backed by `Requests/` + `Support/*Writer`, FtpConfigController (POST `/setup/ftp` + `/setup/ftp/pretest` for FTP/FTPS/SFTP credentials and upload probes), CorsConfigSaveController (POST `/setup/cors`), DkimController, SetupCompletionController (POST `/setup/admin`, `/setup/complete`, `/setup/reset`), DkimGenerator
...
- **SetupMiddleware** — Redirects to /setup when app not configured; bypasses setup/auth/assets/framework flash endpoints via shared SetupAccessTrait → `app/Framework/Middleware/SetupMiddleware.php`
- **SetupGuardMiddleware** — Blocks /setup access once app is configured; reuses SetupAccessTrait for setup state + JSON errors → `app/Framework/Middleware/SetupGuardMiddleware.php`
...
- **HealthReportBuilder** — Shared framework health/readiness report reused by `status`, `/health/live`, `/health/ready` and `/health` → `app/Framework/Health/HealthReportBuilder.php`
...
- **ExportDevelopmentOverlayCommand** (`dev:export-overlay`) — Auto-discovered app command that exports the live auth/RBAC/social-account development snapshot into `boot-core/database/create-catalyst-db.development.sql`, with Docker web-container fallback when host CLI cannot resolve the WSDD DB target → `Repository/App/Surface/Demo/Commands/ExportDevelopmentOverlayCommand.php`
...
- **AppEntryCatalog** — Canonical app-entry catalog shared by root dispatch, `/setup` validation, and settings UI (`Setup`, `User-Access`, public aliases, dev-only entries) → `app/Helpers/Config/AppEntryCatalog.php`
