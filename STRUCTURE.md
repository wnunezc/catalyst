...
- [docs/documentation-contract.md](docs/documentation-contract.md) — hot/warm/cold documentation contract and generated inventory policy
- [docs/runtime-inventory.md](docs/runtime-inventory.md) — Auto-generated symbol, template and script inventory
...
- **Documents module** (`Repository/Framework/Documents/`) — Reusable document templates admin surface: thin web `DocumentTemplateController`, tokenized `DocumentTemplateApiController`, module-specific grid/form/show factories, centralized Requests, transient preview state and preview/export services; shared `work/documents` assets and adoption of `FormBuilder` + `DataGrid`
...
| `Catalyst\Framework\Cli\Commands` | `app/Framework/Cli/Commands/`                    | HelpCommand, VersionCommand, StatusCommand, QualityCheckCommand, AutomationMvcRegressionCommand, DocumentsMvcRegressionCommand, ConfigSecretsSyncCommand, ClaimsListCommand, ClaimsReleaseCommand, ConcurrencySmokeCommand, SensitivitySmokeCommand, TemporalSmokeCommand, IdempotencySmokeCommand, AttachmentsListCommand, AttachmentsSmokeCommand, RetentionRunCommand, RetentionSmokeCommand, ReportingRunCommand, ReportingSmokeCommand, RouteCacheCommand, RouteClearCommand, InspectModulesCommand, InspectModuleCommand, InspectLintCommand, InspectHarnessCommand, DocsSyncRuntimeCommand, DocsInventoryCommand, FixturesAuthCommand, FeatureFlagsListCommand, FeatureFlagsSetCommand, PluginListCommand, PluginToggleCommand, DeployListCommand, DeployRunCommand, TenancyStatusCommand, ApiTokensSmokeCommand, SecurityRegressionCommand, MakeControllerCommand, MakeCrudCommand, MakeModelCommand, MakeMiddlewareCommand, MakeModuleCommand, MakePolicyCommand, MakeRequestCommand |
...
| `Catalyst\Framework\Admin\Grid` | `app/Framework/Admin/Grid/`                      | DataGrid — declarative admin list engine with search, filters, sorting, pagination, row actions, bulk actions, CSV/XLS export and structured cell rendering (`stack`, `code`, `badge`, `badges`, `booleanBadge`) |
...
| `Catalyst\Framework\Documentation` | `app/Framework/Documentation/`                  | RuntimeInventoryGenerator — generated symbol/template/script inventory for documentation contract work |
...
- **DataGrid / DataGridHtmlExportRenderer** *(RM-26)* — Reusable admin listing engine with declarative columns, search, filters, sorting, pagination, row actions, bulk actions, CSV export and template-rendered Excel-compatible XLS export → `app/Framework/Admin/Grid/DataGrid.php`, `app/Framework/Admin/Grid/DataGridHtmlExportRenderer.php`
- **CrudScaffoldService / CrudFileFactory** *(RM-24)* — CLI-facing CRUD generator that emits guarded admin modules, `FormRequest`, entity in `app/Entities/`, migration, routes, `work/{slug}` assets and audit/soft-delete wiring through reusable PHP stubs over the existing module scaffold contract → `app/Framework/Admin/Crud/CrudScaffoldService.php`, `app/Framework/Admin/Crud/CrudFileFactory.php`
...
- **DocsInventoryCommand** (`docs:inventory`) — Generates `docs/runtime-inventory.md` from PHP symbols, templates and canonical scripts; supports `--json`, `--stdout` and `--path` → `app/Framework/Cli/Commands/DocsInventoryCommand.php`
...
- **MakeModuleCommand** (`make:module`) — Scaffolds a full `Repository/{App|Framework}/{Module}/` module with `Controllers/`, structured `Views/` (`pages` + `scope/pages` baseline), `front/`, `lang/`, `routes.php` and `module.php`; route/controller PHP is rendered from stubs, guard-aware surfaces publish initial `work/{slug}` assets immediately and stay coherent with module/permission/navigation registries → `app/Framework/Cli/Commands/MakeModuleCommand.php`
...
- **CLI Stubs** *(16-CLI-Scaffold)* — Template files for controllers, commands, requests, migrations, models, middleware, modules and generated route/module manifests → `app/Framework/Cli/Stubs/`
